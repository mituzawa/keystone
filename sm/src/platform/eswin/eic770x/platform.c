/* EIC770X (HiFive Premier P550) platform hooks.
 * Same no-op hooks as the generic platform; sm_copy_key() is stubbed out
 * because the P550 boot chain (SCPU mask ROM -> QSPI bootchain -> fw_payload)
 * has no stage we control before the SM, so nothing provisions the sanctum_*
 * key material yet. */
#include "../../../enclave.h"
